...
export default function Loja() {
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] dark:bg-[#121212] text-slate-900 dark:text-gray-200 font-sans transition-colors duration-300">
      ...
      {/* HEADER: COBRE NO CLARO (LOGO BRANCA) / GRAFITE NO ESCURO (LOGO ORIGINAL) */}
        ...
        <div className="max-w-[1400px] mx-auto flex justify-between items-center">
          ...
          <Link href="/" className="flex items-center transition-opacity hover:opacity-80">
            <img 
              ...
              alt="Duelo Store" 
              /* 
                MODO CLARO: brightness-0 invert (Força branco total)
                MODO ESCURO: dark:brightness-100 dark:invert-0 (Restaura cores originais)
              */
              className="h-10 md:h-12 w-auto transition-all duration-300 brightness-0 invert dark:brightness-100 dark:invert-0" 
            />
          </Link>
          ...
          <div className="flex-1 max-w-md mx-8 relative hidden md:block">
            ...
            <input 
              ...
              className="w-full pl-12 pr-4 py-2 bg-white/10 border border-white/20 rounded-sm text-[10px] tracking-widest outline-none text-white placeholder:text-white/40 focus:bg-white/20 transition-all"
          ...
          <div className="flex items-center gap-4">
            <ThemeToggle />
            <Link href="/admin" className="text-[9px] font-black tracking-widest text-white/60 hover:text-white transition-colors uppercase">
              Admin
            </Link>
            
            <button 
              onClick={() => setIsCartOpen(true)} 
              className="relative p-2 bg-white/10 border border-white/10 rounded-sm hover:bg-white hover:text-[#CD7F32] dark:hover:text-white dark:hover:bg-[#CD7F32] transition-all"
            >
              <ShoppingBag size={20} />
              {cart.length > 0 && (
                <span className="absolute -top-1 -right-1 bg-white text-[#CD7F32] text-[8px] font-black w-4 h-4 flex items-center justify-center rounded-full shadow-lg">
                  {cart.length}
                </span>
              )}
            </button>
      ...
      <main className="max-w-[1400px] mx-auto p-6 lg:p-10">
        <div className="flex items-center gap-8 mb-12 pb-4 border-b border-slate-200 dark:border-white/5">
          <select onChange={(e) => setFilters({...filters, rarity: e.target.value})} className="bg-transparent text-[10px] font-black uppercase text-[#CD7F32] outline-none">
            <option value="">Raridade</option>
            <option value="Secret Rare">Secret Rare</option>
            <option value="Ultra Rare">Ultra Rare</option>
          </select>
          <select onChange={(e) => setFilters({...filters, lang: e.target.value})} className="bg-transparent text-[10px] font-black uppercase text-[#CD7F32] outline-none">
              ...
              <div key={cat} className="space-y-6">
                ...
                <div className="flex gap-4 overflow-x-auto pb-8 px-2 scrollbar-hide snap-x">
                  {cardsInCategory.map(card => {
                    ...
                    return (
                      <div key={card.id} className="flex-none w-44 rounded-sm flex flex-col transition-all snap-start shadow-md bg-[#CD7F32] dark:bg-[#1E1E1E] border border-transparent dark:border-white/5 hover:shadow-2xl hover:-translate-y-1">
                        <div onClick={() => fetchDetails(card)} className="aspect-[3/4] p-2 bg-white/10 dark:bg-[#161616] relative overflow-hidden cursor-help">
                          <img src={card.image_url} className={`object-contain w-full h-full transition-transform duration-700 hover:scale-110 ${esgotado ? 'grayscale opacity-30' : ''}`} />
                        ...
                        <div className="p-3 flex flex-col flex-1">
                          ...
                          <div className="flex items-center gap-1.5 text-white/70 dark:text-white/30 font-bold text-[8px] uppercase tracking-tighter mb-3">
                          ...
                          <div className="mb-4 flex items-center justify-between">
                            ...
                            <span className="text-[10px] font-black text-white/30 dark:text-white/10">{card.stock}x</span>
                          </div>
                          <button 
            ...
            <div className="w-full md:w-1/2 p-8 md:p-12 space-y-6">
              <div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <span className="bg-[#CD7F32]/10 text-[#CD7F32] text-[9px] font-black px-2 py-0.5 rounded-sm border border-[#CD7F32]/20">{activeLocalCard?.condition}</span>
                </div>
              </div>
      ...
      <aside className={`fixed top-0 right-0 h-full w-full sm:w-80 bg-white dark:bg-[#1E1E1E] text-slate-900 dark:text-white z-[70] transition-transform duration-500 transform ${isCartOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        <div className="h-full flex flex-col p-8">
          ...
          {cart.length > 0 && (
            <div className="pt-8 border-t border-slate-100 dark:border-white/10 space-y-6">
              ...
              <button onClick={handleWhatsApp} className="w-full bg-[#CD7F32] text-white py-5 text-[10px] font-black uppercase tracking-[0.2em] hover:bg-black transition-all">FINALIZAR PEDIDO</button>
            </div>
          )}
        </div>
      </aside>
