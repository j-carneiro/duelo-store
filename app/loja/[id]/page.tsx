...
export default function GaleriaVendedor({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (loading) return <div className="min-h-screen flex items-center justify-center bg-[#F8FAFC] dark:bg-[#121212] text-[#CD7F32]"><Loader2 className="animate-spin" size={32} /></div>;
        ...
        <aside className="w-full lg:w-48 shrink-0 space-y-8">
           ...
           <div className="sticky top-28 space-y-6">
              ...
              {(selRarities.length > 0 || selConditions.length > 0 || selTypes.length > 0 || selSubTypes.length > 0) && (
                <button onClick={() => { setSelRarities([]); setSelConditions([]); setSelTypes([]); setSelSubTypes([]); }} className="text-[8px] font-black text-[#CD7F32] uppercase underline tracking-widest">Limpar Filtros</button>
              )}
           </div>
        </aside>
